...
logger = andy_logger.get_logger("db")
    ...
    def __init__(self):
        if DBUtils.__instance is not None:
            ...
            self.sales_pool = pool.ThreadedConnectionPool(constants.fetch_constant("min_pool"),
                                                          ...
                                                          user=constants.fetch_constant("user"),
        ...
        try:
            ...
            try:
                ...
                if len(sql.split(","))>20:
                    ...
                    logger.info("[%s]:%d - %s" % (caller.filename, caller.lineno, re.sub('\\s+', ' ', cur.mogrify(sql, data).decode())))
